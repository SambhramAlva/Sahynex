...
- A GitHub OAuth App (for GitHub-only login)
...
# Also fill in GITHUB_CLIENT_ID and GITHUB_CLIENT_SECRET for GitHub login
...
|---|---|---|
| `GOOGLE_API_KEY` | ✅ | API key used by the agent runtime |
| `ADK_MODEL` | | Ollama model name (default: `qwen2.5-coder:7b`) |
...
| `GITHUB_OAUTH_REDIRECT_URI` | | Optional callback override (default: `/api/auth/github/callback`) |
...
|---|---|---|
| GET | `/api/auth/github/config` | GitHub OAuth configuration status |
...
| Method | Path | Description |
|---|---|---|
| POST | `/api/webhooks/github` | GitHub Issues webhook receiver for instant issue auto-queueing |
...
Use this when you want new GitHub issues to trigger the agent immediately.

1. Make the backend reachable from GitHub.
2. Set `GITHUB_WEBHOOK_SECRET` in `backend/.env`.
3. In your GitHub repository, open `Settings` → `Webhooks` → `Add webhook`.
4. Set `Payload URL` to `https://YOUR-PUBLIC-URL/api/webhooks/github`.
5. Set `Content type` to `application/json`.
6. Set `Secret` to the same `GITHUB_WEBHOOK_SECRET` value.
7. Choose `Let me select individual events` and enable only `Issues`.
8. Save the webhook and create a new issue to test instant triggering.

The backend verifies GitHub signatures using `X-Hub-Signature-256` before queueing the issue.

## Switching To A Permanent Webhook URL

Once you deploy your backend to a stable domain (for example `https://api.yourdomain.com`), update the repository webhook URL to:

`https://api.yourdomain.com/api/webhooks/github`

If you are using the local file DB and want a quick helper command, you can run:

`python backend/scripts/update_webhook_url.py --url https://api.yourdomain.com/api/webhooks/github`
